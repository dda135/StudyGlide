...
public class DrawableTypeRequest<ModelType> extends DrawableRequestBuilder<ModelType> implements DownloadOptions {
    //来源于Glide构造方法当中注册的ModelLoaderFactory
    private final ModelLoader<ModelType, InputStream> streamModelLoader;
    private final ModelLoader<ModelType, ParcelFileDescriptor> fileDescriptorModelLoader;
    //RequestManager中的OptionsApplier，主要是保证build的一致性
    ...
    private static <A, Z, R> FixedLoadProvider<A, ImageVideoWrapper, Z, R> buildProvider(Glide glide,
        ...
        //这里其实和ModelLoader是一样的，在Glide里面默认有注册Transcoder
        if (transcoder == null) {
            transcoder = glide.buildTranscoder(resourceClass, transcodedClass);
        }
        //这里其实和ModelLoader是一样的，在Glide里面默认有注册DataProvider
        DataLoadProvider<ImageVideoWrapper, Z> dataLoadProvider = glide.buildDataProvider(ImageVideoWrapper.class,
                resourceClass);
        //通过在RequestManager中通过Glide默认注册的ModelLoaderFactory获得的ModelLoader构建一个统一的ModelLoader
