...
public class Glide {
    ...
    private final Engine engine;//都是通过Glide获得，因Glide为单例，所以Engine也为单例
        ...
        if (glide == null) {
            synchronized (Glide.class) {
                if (glide == null) {
                    ...
                    //如果允许使用GlideModule，尝试解析AndroidManifest中指定的GildeModule
                    List<GlideModule> modules = parseGlideModules(applicationContext);
                    //依次回调GlideModule中的applyOptions
                    for (GlideModule module : modules) {
                        module.applyOptions(applicationContext, builder);
                    }
                    //通过工厂构建新的Glide
                    glide = builder.createGlide();
                    //依次回调GlideModule中的registerComponents
                    for (GlideModule module : modules) {
     ...
     * 在允许使用Module的情况下，解析AndroidManifest中指定的GlideModule
     */
    ...
    Glide(Engine engine, MemoryCache memoryCache, BitmapPool bitmapPool, Context context, DecodeFormat decodeFormat) {
        ...
        //注册对应元素的处理工厂
        //Glide通过泛型发起请求，然后找到指定的工厂
        //注意到其实绝大部分最后都指向GlideUrl的工厂
    ...
    /**
     * 通过ImageViewTarget的工厂根据解析资源类型获取指定的Target
     */
    ...
    public static <T, Y> ModelLoader<T, Y> buildModelLoader(Class<T> modelClass, Class<Y> resourceClass, Context context) {
        if (modelClass == null) {
            if (Log.isLoggable(TAG, Log.DEBUG)) {Log.d(TAG, "Unable to load null model, setting placeholder only");}
            return null;
        }
